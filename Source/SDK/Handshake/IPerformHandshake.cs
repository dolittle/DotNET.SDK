...
public interface IPerformHandshake
{
    ...
    /// <param name="buildResults">The <see cref="IEnumerable{T}"/> of <see cref="ClientBuildResult"/>.</param>
    /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
    /// <returns>A <see cref="Task{TResult}"/>that, when resolved, returns the <see cref="HandshakeResult"/>.</returns>
    Task<HandshakeResult> Perform(uint attemptNum, TimeSpan timeSpent, Version headVersion, IEnumerable<ClientBuildResult> buildResults, CancellationToken cancellationToken);
}
