...
{
    /// <summary>
    /// Represents an unique identifier for an <see cref="IEvent"/> that has been committed to the <see cref="IEventStore"/>.
    /// </summary>
    /// <remarks>
    /// The identifier is constructed from the <see cref="Microservice"/> and <see cref="TenantId"/> that committed the event, and the <see cref="EventLogSequenceNumber"/> it got in the event log.
    /// </remarks>
    public class EventIdentifier : ConceptAs<string>
    {
        ...
        /// </summary>
        /// <param name="microservice">The <see cref="Microservice"/> that committed the event.</param>
        ...
        public EventIdentifier(Microservice microservice, TenantId tenant, EventLogSequenceNumber sequenceNumber)
        {
            Value = Compose(microservice, tenant, sequenceNumber);
        }
        ...
        /// </remarks>
        public static string Compose(Microservice microservice, TenantId tenant, EventLogSequenceNumber sequenceNumber)
        {
        ...
        /// <param name="tenant">When this method returns, the <see cref="TenantId"/> that committed the event.</param>
        /// <param name="sequenceNumber">When this method returns, the <see cref="EventLogSequenceNumber"/> of the event that was committed.</param>
        public void Decompose(out Microservice microservice, out TenantId tenant, out EventLogSequenceNumber sequenceNumber)
        {
