...
public record PartitionId(string Value) : ConceptAs<string>(Value)
{
    ...
    public static implicit operator PartitionId(Guid partitionId) => new(partitionId.ToString());
    ...
    /// <returns>The converted <see cref="PartitionId"/>.</returns>
    public static implicit operator PartitionId(string partitionId) => new(partitionId);
}
