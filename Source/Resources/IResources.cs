...
namespace Dolittle.SDK.Resources
{
    ...
    public interface IResources
    {
        ...
        /// </summary>
        /// <param name="tenantId">The <see cref="TenantId"/> to get the resource for.</param>
        /// <param name="createSettingsFromConnectionString">The <see cref="Func{T1, TResult}"/>that creates the <see cref="MongoClientSettings"/> that can yse the connection string.</param>
        /// <param name="databaseSettings">The optional <see cref="MongoDatabaseSettings"/>.</param>
        /// <param name="cancellationToken">The <see cref="CancellationToken"/>.</param>
        /// <returns>A <see cref="Task"/> representing the asynchronous operation.</returns>
        /// <exception cref="FailedToGetResource">Exception that gets thrown when the Runtime failed getting the resource.</exception>
        Task<IMongoDatabase> GetMongoDB(TenantId tenantId, Func<MongoUrl, MongoClientSettings> createSettingsFromConnectionString, MongoDatabaseSettings databaseSettings = default, CancellationToken cancellationToken = default);
    }
}