...
{
    public class events_with_implicit_event_type : given.two_aggregate_roots
    {
        static IEnumerable<UncommittedAggregateEvent> uncommitted_events;
        Because of = () =>
        {
            statefull_aggregate_root.Apply(first_event);
            statefull_aggregate_root.Apply(second_event);
            statefull_aggregate_root.Apply(third_event);
