...
namespace Dolittle.SDK.Events.for_AggregateResponseToSDKConverter.when_converting_a_commit_aggregate_events_response
{
    public class and_commit_succeeded : given.a_converter_and_a_protobuf_execution_context
    {
        ...
        Because of = () => try_result = converter.TryToSDK(commit_aggregate_events_response, out committed_events, out exception);
        ...
        It should_have_called_the_serializer_with_the_content = () =>
            serializer.Verify(_ => _.TryToDeserialize(commit_aggregate_events_response.Events.Events[0].Content, Moq.It.IsAny<EventType>(), out Moq.It.Ref<object>.IsAny, out Moq.It.Ref<Exception>.IsAny), Times.Once());
    }
}
